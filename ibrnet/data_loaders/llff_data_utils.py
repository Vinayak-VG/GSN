...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, dinofield=False, dino_type='v1', folder_name = "DiNOFeats"):
    ...
    if dinofield:
        if dino_type == 'v1':
            dinodir = os.path.join(basedir, folder_name)
        elif dino_type == 'v2':
            dinodir = os.path.join(basedir, "DiNOFeats_v2")
    ...
    if dinofield:
        dinofiles = [
            os.path.join(dinodir, f)
            for f in sorted(os.listdir(dinodir))
            if f.endswith("pt")
        ]
    ...
    if not load_imgs:
        ...
        if dinofield:
            dinofts = [load_features(file=dinofile, imhw=(378, 504)) for dinofile in dinofiles]
            dinofts = np.stack(dinofts, -1)
            return poses, bds, imgs, imgfiles, dinofts, dinofiles
...
def load_llff_data(
    ...
    dinofield=False,
    dino_type='v1',
    folder_name = "DiNOFeats"
):

    # quit()
    
    out = _load_data(
        basedir, factor=factor, load_imgs=load_imgs, dinofield=dinofield, dino_type=dino_type, folder_name = folder_name
    )  # factor=8 downsamples original imgs by 8x
    if out is None:
        return
    else:
        if dinofield:
            poses, bds, imgs, imgfiles, dinofts, dinofiles = out
        else:
    ...
    if imgs is not None:
        ...
        images = None
    if dinofield:
        if dinofts is not None:
            dinofts = np.moveaxis(dinofts, -1, 0).astype(np.float32)
            dinofeats = dinofts
            dinofeats = dinofeats.astype(np.float32)
        else:
            dinofeats = None
    ...
    if dinofield:
        return images, poses, bds, render_poses, i_test, imgfiles, dinofeats, dinofiles
    else:
