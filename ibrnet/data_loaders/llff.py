...
import sys
from ibrnet.data_loaders.load_features import load_features
sys.path.append("../")
...
class LLFFDataset(Dataset):
    def __init__(self, args, mode, **kwargs):
        base_dir = os.path.join("/mnt/data/sushanth/vinayak/data/real_iconic_noface/")
        self.args = args
        self.mode = mode  # train / test / validation
        self.num_source_views = args.num_source_views
        self.dinofield = args.dinofield
        self.render_rgb_files = []
        if args.dinofield:
            self.render_dino_files = []
        ...
        for i, scene in enumerate(scenes):
            ...
            if args.dinofield:
                _, poses, bds, render_poses, i_test, rgb_files, _, dino_files = load_llff_data(
                    scene_path, load_imgs=False, factor=4, dinofield=args.dinofield, dino_type=args.dino_type, folder_name=args.folder_name,
                )
            else:
            ...
            if args.dinofield:
                self.train_dino_files.append(np.array(dino_files)[i_train].tolist())

            num_render = len(i_render)
            self.render_rgb_files.extend(np.array(rgb_files)[i_render].tolist())
            if args.dinofield:
                self.render_dino_files.extend(np.array(dino_files)[i_render].tolist())
    ...
    def __getitem__(self, idx):
        ...
        if self.dinofield:
            dino_file = self.render_dino_files[idx]
            dinofeats = load_features(file=dino_file, imhw=(rgb.shape[0], rgb.shape[1]))
        ...
        if self.dinofield:
            train_dino_files = self.train_dino_files[train_set_id]
        ...
        for id in nearest_pose_ids:
            ...
            if self.dinofield:
                src_dino = load_features(file=train_dino_files[id], imhw=(378, 504))
                src_dinos.append(src_dino)
        ...
        if self.dinofield:
            src_dinos = np.stack(src_dinos, axis=0)
        ...
        if self.dinofield:
            return {
                ...
                "dino": dinofeats.squeeze(-1) if self.dinofield else None,
                "camera": torch.from_numpy(camera),
                "rgb_path": rgb_file,
                "src_rgbs": torch.from_numpy(src_rgbs[..., :3]),
                "src_dinos": src_dinos if self.dinofield else None,
