...
class Projector:
    def __init__(self, device, dinofield=False):
        self.device = device
        self.dinofield = dinofield
    ...
    def compute(self, xyz, query_camera, train_imgs, train_cameras, dinofeats, featmaps):
        ...
        if self.dinofield:
            dinofeats = dinofeats.squeeze(0).squeeze(-1)
            dinofeats = dinofeats.permute(0, 3, 1, 2)
        ...
        if self.dinofield:
            dinos_sampled = F.grid_sample(dinofeats, normalized_pixel_locations, align_corners=True)
            dino_sampled = dinos_sampled.permute(2, 3, 0, 1)
    
        if self.dinofield:
            rgb_feat_sampled = torch.cat(
                [rgb_sampled, feat_sampled, dino_sampled], dim=-1
