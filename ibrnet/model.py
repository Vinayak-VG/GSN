...
from ibrnet.transformer_network import TransIBRNet, DiNOPredictor
...
class IBRNetModel(object):
    def __init__(self, args, load_opt=True, load_scheduler=True):
        ...
        if args.net == "mlp_ibr":
            ...
            if args.dinofield:
                self.dino_coarse = DiNOPredictor(args, posenc_dim=3 + 3 * 2 * 10).to(device)
        ...
        if self.net_fine is not None:
            ...
            if self.args.dinofield:
                self.optimizer = torch.optim.Adam(
                    [
                        {"params": self.net_coarse.parameters()},
                        {"params": self.feature_net.parameters(), "lr": args.lrate_feature/10},
                        {"params": self.dino_coarse.parameters(), "lr": args.lrate_feature},

                    ],
                    lr=args.lrate_mlp/10,
                )
            else:
                self.optimizer = torch.optim.Adam(
                    [
        ...
        if args.distributed:
            if self.args.dinofield:
                self.dino_coarse = torch.nn.parallel.DistributedDataParallel(
                    self.dino_coarse, device_ids=[args.local_rank], output_device=args.local_rank
                )
    ...
    def save_model(self, filename):
        if self.args.dinofield:
            to_save = {
                ...
                "dino": de_parallel(self.dino_coarse).state_dict(),
            }
        else:
            to_save = {
                "optimizer": self.optimizer.state_dict(),
    ...
    def load_model(self, filename, load_opt=True, load_scheduler=True, dino_load=True):
        ...
        if self.args.dinofield:
            try:
                self.dino_coarse.load_state_dict(to_load["dino"])
                if load_opt:
