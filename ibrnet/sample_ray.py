...
class RaySamplerSingleImage(object):
    def __init__(self, args, data, device, resize_factor=1, render_stride=1):
        super().__init__()
        ...
        if resize_factor != 1:
            ...
            if self.dino is not None:
                self.dino = F.interpolate(
                    self.dino.permute(0, 3, 1, 2), scale_factor=resize_factor
                ).permute(0, 2, 3, 1)
        ...
        if self.dino is not None:
            self.dino = self.dino.reshape(-1, args.dino_dim)
    ...
    def get_all(self):
        ret = {
            ...
            "dino": self.dino.cuda() if self.dino is not None else None,
            "src_rgbs": self.src_rgbs.cuda() if self.src_rgbs is not None else None,
            "src_dinos": self.src_dinos.cuda() if self.args.dinofield else None,
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        ret = {
            ...
            "dino": dino.cuda() if dino is not None else None,
            "src_rgbs": self.src_rgbs.cuda() if self.src_rgbs is not None else None,
            "src_dinos": self.src_dinos.cuda() if self.args.dinofield else None,
            "src_cameras": self.src_cameras.cuda() if self.src_cameras is not None else None,
