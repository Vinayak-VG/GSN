...
def parse_args():
    ...
    parser.add_argument("--main_folder", default='../../../../../media/Data1/vinayak/dtu/mvs_training/dtu', type=str)
...
def main(args=None):
    scenes = sorted(os.listdir(os.path.join(args.main_folder, 'Rectified')))
    model = get_model(args.model, args.model_path, f"cuda:{args.gpu}")
    # model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14')
    # model = DINO()
    
    for scene in tqdm.tqdm(scenes):
        args.dir_images = os.path.join(args.main_folder, 'Rectified', scene)
        ...
        os.makedirs(os.path.join(args.main_folder, 'DiNOFeats', scene), exist_ok=True)
        
        for idx in range(len(all_filenames)):
            output_path_pca = os.path.join(args.main_folder, 'DiNOFeats', scene, os.path.basename(all_filenames[idx].split('.')[0]) + ".pt")
