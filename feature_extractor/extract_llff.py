...
def main(args=None):
    ...
    for scene in tqdm.tqdm(scenes):
        ...
        for batch, filenames in dataloader:
            ...
            # batch_new = F.interpolate(batch_feats, scale_factor=8,
            #                     mode="bilinear", align_corners=True)
            # batch_new = batch_new[0, :3].permute(1, 2, 0).cpu()
            # batch_new = (batch_new - torch.min(batch_new))/(torch.max(batch_new) - torch.min(batch_new))
            # imageio.imwrite(f'dinov1_first.png', (batch_new.detach().cpu().numpy()*255).astype('uint8'))
        ...
        # batch_new = F.interpolate(X, scale_factor=8,
        #                         mode="bilinear", align_corners=True)
        # batch_new = batch_new[0, :3].permute(1, 2, 0).cpu()
        # batch_new = (batch_new - torch.min(batch_new))/(torch.max(batch_new) - torch.min(batch_new))
        # imageio.imwrite(f'dinov1_pca.png', (batch_new.detach().cpu().numpy()*255).astype('uint8'))
        # quit()
        
        os.makedirs(os.path.join(args.main_folder, scene, 'DiNOFeats'), exist_ok=True)
        pk.dump(pca, open(f"pca/llff_dino_pca{scene}.pkl","wb"))
        
        for idx in range(len(all_filenames)):
            output_path_pca = os.path.join(args.main_folder, scene, 'DiNOFeats', os.path.basename(all_filenames[idx].split('.')[0]) + ".pt")
