...
src = "../../../../../media/Data1/vinayak/dtu/mvs_training/dtu"
scenes = sorted(os.listdir(os.path.join(src, "Rectified")))
...
for scene in tqdm.tqdm(range(len(scenes))):
    ...
    os.makedirs(os.path.join(src, 'CLIPFeats', scenes[scene]), exist_ok=True)
    features = torch.zeros((len(images), 9, 12, len(model_names), 512)).cuda()
    for model_num in range(len(model_names)):
        ...
        for image in range(len(images)):
            image_path = os.path.join(src, 'Rectified', scenes[scene], images[image])
